...
# План разработки версии 0.09: Разделение ресурсов и строительных блоков

**Цель:** Улучшить механику строительства, явно разделив ресурс "Камень" и "Каменную стену" (строительный блок). Это добавит глубины в механику крафта и использования ресурсов, а также решит проблему некорректного определения "дома" ИИ.

**Ключевые нововведения:**

1.  **Новый строительный блок: "Каменная стена"**: Отдельный от ресурса "Камень" предмет, который используется для строительства. Она будет иметь уникальный внешний вид (`█`) и свойства.
2.  **Механика крафта "Каменной стены"**: Игрок (и ИИ) будет использовать определенное количество ресурса "Камень" для создания одной "Каменной стены".
3.  **Обновление инвентаря**: Инвентарь будет хранить оба типа предметов: "Камень" (сырье) и "Каменная стена" (строительный блок).
4.  **Изменение строительства**: Действие строительства будет использовать "Каменную стену" из инвентаря.
5.  **Обновление ИИ-строителя**: ИИ будет понимать новые правила: собирать "Камень", крафтить "Каменные стены" и использовать их для постройки убежищ.
6.  **Явное различие проходимости**: `ResourceTile` для "Камня" будет проходимым, `WallTile` для "Каменной стены" - непроходимым.

### Подробный план реализации:

**Шаг 1: Определение нового строительного блока "Каменная стена"**
*   **Модификация:** `MapCell.cs`
*   **Изменения:**
    *   Добавить новый статический фабричный метод `MapCell.BuiltStoneWall()`, который будет создавать `MapCell` со следующими свойствами:
        *   `DisplayChar = '█'` (для визуального отличия от `#` обычных стен или ресурсов).
        *   `CellColor = new Color(100, 100, 100, 255)` (пример, цвет может быть скорректирован).
        *   `_baseIsWall = true` (делает эту клетку непроходимой).
    *   **Уточнение:** `ResourceTile` (например, для "Stone") должен быть проходимым (`_baseIsWall = false` для соответствующей `MapCell`), чтобы он не определялся как часть замкнутого контура для `IsInside()`. Только `MapCell`, созданные как `MapCell.BuiltStoneWall()` или `MapCell.Wall()`, должны быть непроходимыми.
*   **Назначение:** Четкое разграничение между добываемым ресурсом и строительным материалом на уровне карты.

**Шаг 2: Механика крафта "Каменной стены" из "Камня"**
*   **Модификация:** `Player.cs`
*   **Изменения:**
    *   Добавить новый публичный метод `public bool Craft(string itemToCraft, Dictionary<string, int> recipe)`:
        *   Он будет проверять наличие необходимых ресурсов (`recipe`) в `PlayerInventory`.
        *   Если ресурсов достаточно, они будут удалены из инвентаря, а `itemToCraft` будет добавлен.
        *   Вернет `true` при успешном крафте, `false` в противном случае.
*   **Модификация:** `ManualPlayerController.cs`
*   **Изменения:**
    *   Добавить обработку новой клавиши (например, `KeyboardKey.KEY_C`).
    *   При нажатии вызывать `player.Craft("StoneWall", new Dictionary<string, int> { { "Stone", 2 } })` (рецепт: 2 "Stone" -> 1 "StoneWall").
*   **Назначение:** Введение механики обработки ресурсов и возможность получения строительных блоков.

**Шаг 3: Обновление механики строительства**
*   **Модификация:** `Player.cs`
*   **Изменения:**
    *   Метод `public bool Build(GameMap map, string buildingBlockType, int targetX, int targetY)` будет изменен:
        *   Он будет проверять `PlayerInventory` на наличие `buildingBlockType` (например, `"StoneWall"`).
        *   Если есть, уменьшает количество `buildingBlockType` в инвентаре.
        *   Заменяет целевой тайл на карте на соответствующий строительный блок, используя `MapCell.BuiltStoneWall()` (в случае `"StoneWall"`).
    *   Метод `public bool BuildDoor(GameMap map, int targetX, int targetY)` также будет изменен, чтобы требовать `buildingBlockType` (например, `"StoneWall"`) из инвентаря для установки двери.
*   **Назначение:** Корректное использование нового строительного блока "Каменная стена" для построек.

**Шаг 4: Обновление логики ИИ-строителя**
*   **Модификация:** `AIPlayerController.cs`
*   **Изменения:**
    *   **В `CollectingResources` (для строительства убежища):**
        *   ИИ будет проверять, достаточно ли у него в инвентаре `"StoneWall"` для постройки.
        *   Если нет, ИИ будет собирать `"Stone"`.
        *   Когда будет достаточно `"Stone"`, ИИ будет вызывать `player.Craft("StoneWall", ...)` для получения необходимых строительных блоков.
        *   Только после этого ИИ перейдет к следующему этапу - строительству.
    *   **В `BuildingWalls` (для строительства убежища):**
        *   Логика будет изменена для вызова `player.Build(map, "StoneWall", target.X, target.Y)` вместо `player.Build(map, "Stone", ...)`.
    *   **В `BuildActionType` и `BuildAction` (если необходимо):** `BuildAction` будет хранить имя `buildingBlockType` (например, `"StoneWall"`), которое будет использоваться при строительстве.
    *   **Случайное строительство (если есть):** Любая другая логика, где ИИ сейчас строит, используя "Stone", будет изменена для использования "StoneWall".
*   **Назначение:** Адаптация ИИ к новой механике строительства и корректное выполнение задач.

**Шаг 5: Обновление тестов**
*   **Модификация:** `tests/GGD.Tests/PlayerTests.cs`, `tests/GGD.Tests/AIPlayerControllerTests.cs`
*   **Изменения:**
    *   Создать новые юнит-тесты для `Player.Craft()`: проверить, что ресурсы расходуются, а крафченный предмет появляется в инвентаре.
    *   Обновить существующие тесты для `Player.Build()`: убедиться, что он теперь требует `"StoneWall"` и корректно устанавливает `MapCell.BuiltStoneWall()`.
    *   Обновить тесты для `AIPlayerControllerTests`: убедиться, что ИИ правильно собирает "Камень", крафтит "Каменные стены" и использует их для строительства убежищ.
*   **Назначение:** Обеспечение качества кода нового функционала и подтверждение корректной работы всех изменений.
...
*   **Версия 0.09:** Завершена (разделение ресурсов и стройблоков, крафт каменной стены).
